...
domain, fix a unit vector $e\in\R^d$, and set
$\Omega_L=(D-\frac L2e)\cup(D+\frac L2e)$.  For the restricted fractional
Laplacian $(-\Delta)^s$, $0<s<1$, let $\mu_1$ be the one-well ground-state
eigenvalue, let $\phi_1$ be its positive $L^2$-normalized eigenfunction, and
put $m_1=\int_D\phi_1$.  As $L\to\infty$,
\begin{align*}
 \lambda_1(\Omega_L)
 ...
 &=\mu_1+c_{d,s}m_1^2L^{-d-2s}
   +O\!\left(L^{-d-2s-2}+L^{-2d-4s}\right),
\end{align*}
where $c_{d,s}$ is the singular-integral normalization.  For two equal balls,
the second formula gives the leading rate in the nonlocal
Hong--Krahn--Szeg\H{o} minimizing sequence.  More generally, for any bounded
Lipschitz reference domain and fixed distinct translation sites
$a_1,\dots,a_N$, the first $N$ eigenvalues are
\[
...
computations on two and three intervals illustrate these fixed-mesh limits.
All implicit constants are uniform in $L$ for fixed $d,s,D,R$ and, in the
multi-well statement, fixed $N$ and translation sites.
...
\texttt{paper-2026-09-04-r1}.  The tagged development source is available at
\href{https://github.com/skypher/laplace-tunneling/tree/paper-2026-09-04-r1}%
{github.com/skypher/laplace-tunneling/tree/paper-2026-09-04-r1}.
\end{sloppypar}
