...
\hypersetup{
  ...
  pdftitle={Algebraic Tunnelling for the Restricted Fractional Laplacian on Distant Domains},
...
\title{Algebraic tunnelling for the restricted fractional\\
...
\newcommand{\shorttitle}{ALGEBRAIC TUNNELLING ON DISTANT DOMAINS}
...
\begin{abstract}
Let $D\subset\R^d$ be a bounded centrally symmetric Lipschitz domain, and
let $\Omega_L$ be the union of two reflected translates of $D$ whose centers
are separated by $L$.  For the restricted fractional Laplacian
$(-\Delta)^s$, $0<s<1$, we determine the leading splitting of the two
...
\[
 ...
 =\mu_1+c_{d,s}m_1^2L^{-d-2s}
   +O\!\left(L^{-d-2s-2}+L^{-2d-4s}\right).
\]
The proof uses an exact reflection reduction to the two operators
$A_D\pm B_L$, an elementary isolated-eigenvalue estimate, and a second-order
Taylor expansion of the interaction kernel.  For two equal balls, the upper
formula gives the leading rate in the nonlocal Hong--Krahn--Szeg\H{o}
minimizing sequence.  More generally, for any fixed collection of distinct
centers $a_1,\dots,a_N$, the first $N$ eigenvalues are
\[
...
integral kernel.  This differs from the local Dirichlet Laplacian, whose
spectrum on a disconnected union is exactly the multiset union of the
component spectra.  Two distant congruent components therefore form a
nonlocal analogue of a double well: their limiting degeneracies split at an
algebraic, rather than exponential, scale.
...
\cite[Theorem~6.2]{BrascoParini2016}.  Parini and Salort placed such distant
components in a general concentration--compactness dichotomy
\cite[Theorems~1.2 and~1.3]{PariniSalort2020}.  Together these works establish
the qualitative limiting and dichotomy picture but do not specify the leading
interaction coefficient.  The purpose of this article is to extract that
coefficient and the associated effective finite-dimensional interaction.
...
\cite[Section~10, equations~(10.3) and~(10.7)]{Zahl2026}.  Wu subsequently
settled Zahl's no-local-minimum conjecture for that discrete signed-mass
energy \cite[abstract]{Wu2026}.  In another recent direction, Dipierro, Proietti Lippi,
...
Neither work derives a large-separation spectral expansion for the ordinary
restricted fractional Laplacian.  Zahl's discrete energy anticipates the
matrix in \eqref{eq:effective-matrix}; the result here derives it from the full
restricted fractional operator, bounds the off-diagonal block in operator
norm, controls its coupling to the complementary one-well modes through the
spectral gap, and identifies every ordered eigenvalue in the cluster.  We are
...
explicit weighted interaction matrix.  Both proofs are self-contained after
the standard one-well spectral facts.  The interval provides an explicit
benchmark: one-well eigenvalue and eigenfunction estimates are available in
...
\begin{equation}\label{eq:form}
 ...
 \qquad u\in\Hs(G),
\end{equation}
where $c_{d,s}>0$ is the normalization in the singular-integral definition
of $(-\Delta)^s$.  We denote the associated self-adjoint operator on
$L^2(G)$ by $A_G$.
...
from \cite[Theorems~5.4 and~7.1]{DiNezzaPalatucciValdinoci2012}.  Write the first two
eigenvalues of $A_D$ as
\[
 \mu_1<\mu_2,
 \qquad g=\mu_2-\mu_1>0.
\]
...
\begin{lemma}[Exact reflection reduction]\label{lem:block}
...
$L^2(D)\oplus L^2(D)$.  Translational and reflection invariance identify the
two diagonal parts of the form with $\mathcal q_D[u]$ and
$\mathcal q_D[v]$.  Positive separation permits smooth compactly supported
cutoffs isolating the two components.  Multiplication by these cutoffs is
bounded on $H^s(\R^d)$, while conversely the sum of separated translated and
reflected one-well functions lies in $H^s(\R^d)$.  Hence the unitary
identifies $\Hs(\Omega_L)$ with $\Hs(D)\oplus\Hs(D)$.  The cross-kernel is
bounded by positive separation.  Expanding the square in
\eqref{eq:form}, the difference
between the form of the sum and the two separate forms is exactly
\[
 -2c_{d,s}\operatorname{Re}\iint_{D\times D}
 \frac{u(x)\overline{v(y)}}{\abs{Le-x-y}^{\kappa}}\,dx\,dy
 =2\operatorname{Re}\ip{u}{B_Lv}.
\]
This proves the block representation in the sense of closed forms.  Since
...
\begin{proof}
The upper bound follows by testing the Rayleigh quotient with $\phi$.  For a
normalized vector $\psi=a\phi+\eta$, where $\eta\perp\phi$ and
$t=\norm{\eta}$, the spectral gap gives
\[
 \ip{\psi}{A\psi}\geq\mu+gt^2.
\]
Since $\abs{w}\leq b$ and $\abs{a}\leq1$,
\begin{align*}
 \ip{\psi}{(A+W)\psi}-(\mu+w)
 &\geq gt^2-2bt^2-2bt\\
 &=(g-2b)t^2-2bt\\
...
centers are separated by $L$.  Then, as $L\to\infty$,
\[
...
The two-well reflection decomposition is special to a pair.  For finitely
...
$\eta_N\leq b\leq g/4$ and $g/2-b\geq g/4$, the first $N$ eigenvalues of
this comparison operator are the $N$ eigenvalues of its $P$-block.  Another
...
\]
Using $F(0)=L^{-\kappa}\abs a^{-\kappa}$ in
...
Let $D$, $\mu_1$, $g$, $\phi_1$, and $m_1$ be as in Section~2, and let the
multi-well geometry be as in Definition~\ref{def:multi-geometry}.  If
...
\begin{equation}\label{eq:multi-next-level}
...
\eqref{eq:multi-next-level} follows from \eqref{eq:cluster-separation}.
...
For $N=2$, $a_1=-e/2$, and $a_2=e/2$, the eigenvalues of
$M_{\mathbf a}$ are $-c_{d,s}m_1^2$ and $c_{d,s}m_1^2$.  The bound
\eqref{eq:multi-eigenvalue-bound} then agrees with
...
For every multi-well configuration, $\theta_1<0$ is simple and has an
eigenvector with strictly positive entries.  Moreover, $\theta_N>0$.
Consequently, for all sufficiently large $L$, the lowest eigenvalue in the
cluster is simple and lies below $\mu_1$, while at least one cluster
eigenvalue lies above $\mu_1$.
\end{corollary}

\begin{proof}
The off-diagonal entries of $M_{\mathbf a}$ are strictly negative.  Its
Rayleigh quotient at $(1,\dots,1)$ is negative, so $\theta_1<0$.  If $z$ is a
minimizer of the Rayleigh quotient, replacing $z$ by its componentwise
absolute value cannot increase that quotient, and equality forces all
nonzero components to have the same sign.  The eigenvalue equation then
shows that no component can vanish.  Thus every eigenvector for $\theta_1$
has one strict sign.  Two linearly independent such eigenvectors would have
a nonzero linear combination with a zero component, so $\theta_1$ is simple.
Finally, $\operatorname{tr}M_{\mathbf a}=0$ and $\theta_1<0$, hence
$\theta_N>0$.  Put
\[
 r_L=\varepsilon_L+\frac{2\gamma_L^2}{g}.
\]
Theorem~\ref{thm:multi-well} gives $L^\kappa r_L\to0$.  Since $\theta_1$ is
simple, $\theta_2-\theta_1>0$.  Choose $L$ so large that
\[
 L^\kappa r_L
 <\min\left\{\frac{\theta_2-\theta_1}{2},-\theta_1,\theta_N\right\}.
\]
Then \eqref{eq:multi-eigenvalue-bound} gives
$\lambda_2-\lambda_1>0$, $\lambda_1<\mu_1$, and
$\lambda_N>\mu_1$, which proves all three assertions for the full operator.
\end{proof}

\begin{corollary}[Regular-simplex cluster]\label{cor:simplex}
Suppose the centers satisfy $\abs{a_i-a_j}=r$ for all $i\ne j$, as for the
vertices of a regular simplex.  Put
\[
...
three-well limits.
It is included as a reproducibility test of the formulas, not as an input to
their proofs.  Take $d=1$, $0<s<1/2$, and partition each unit interval into
cells of length $h$.  Proposition~\ref{prop:cell-matrix} shows directly that
cell indicators have finite form energy in this range, so they span a
conforming finite-dimensional subspace of the form domain.

\begin{proposition}[Exact cell-integral matrix]\label{prop:cell-matrix}
Let $I_1,\dots,I_M$ be pairwise disjoint cells of length $h$, set
$G=\bigcup_{\alpha=1}^M I_\alpha$, let $x_\alpha$ be their centers, put
$r_{\alpha\beta}=\abs{x_\alpha-x_\beta}$, and set $q=1-2s$.  In the
...
\begin{proof}
For two disjoint cells with center distance $r\geq h$, direct integration
twice gives
\[
...
$L^2$-normalized piecewise-constant eigenfunction of its Galerkin matrix.  Put
$m_{1,h}=\int_D\phi_{1,h}$.  For the corresponding Galerkin matrix on
$\bigcup_{j=1}^N(D+La_j)$, write its ordered eigenvalues as
$\lambda_{k,h}(L)$.  Then, as $L\to\infty$, for fixed distinct
$a_1,\dots,a_N$,
...
On the 180-cell mesh, the fixed-mesh effective matrices give
\begin{align*}
 \text{two wells:}&\quad (-0.18521477,0.18521477),\\
 \text{centers }(-L,0,L):&\quad
...
The proof architecture was developed with assistance from the OpenAI GPT-5.6
Sol model (\texttt{gpt-5.6-sol}) at max reasoning effort.  The system was used as an
interactive research assistant to develop and stress-test the argument,
audit citations and calculations, prepare exact verification code, and
revise the manuscript.  The author directed the research, selected and
checked the final arguments, and takes responsibility for all mathematical
claims, citations, computations, and wording.

\section*{Code availability}

The script \path{tools/check_asymptotics.py} reconstructs every entry in
Tables~\ref{tab:mesh} and \ref{tab:clusters}.  It checks the cell-integral
identity before solving the eigenproblems and prints progress after each
separation.  The pinned numerical dependencies are listed in
\path{requirements-numerics.txt}.  The versioned source package is available
at \href{https://github.com/skypher/laplace-tunneling}%
