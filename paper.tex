...
decomposition into symmetric and antisymmetric sectors and an explicit
separation threshold.  For fixed distinct sites $a_1,\ldots,a_N$, let
...
symmetry.  Both arguments are self-contained after the standard one-well
spectral facts.
...
\section{Reproducible fixed-mesh checks on intervals}\label{sec:numerics}
...
$(-L/2,L/2)$, while the three-well centers are $(-L,0,L)$.  The corresponding
fixed-mesh effective-matrix spectra, indexed by the fixed sites, are
\begin{align*}
...
mesh.  At $L=3$, the discrete bounds are $\gamma_L=0.10858$ for two wells and
$\gamma_L=0.21716$ for three wells, both below $g_h/4=0.22424$.  Since
...
\begin{sloppypar}
The ancillary files contain the script \path{check_asymptotics.py}, the
pinned dependencies in \path{requirements-numerics.txt}, and the reference
output \path{asymptotics_180.txt} reproducing Tables~\ref{tab:mesh} and
\ref{tab:clusters}.  The output was reproduced using
Python~3.12.3, NumPy~1.26.4, and SciPy~1.11.4.  These files are archived with
the manuscript; the corresponding source is available in the tagged release
\href{https://github.com/skypher/laplace-tunneling/tree/paper-2026-09-05}%
{\texttt{paper-2026-09-05}}.
\end{sloppypar}
