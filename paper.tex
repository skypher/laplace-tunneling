...
\begin{abstract}
Let $D\subset B_R(0)\subset\mathbb R^d$ be a bounded centrally symmetric
Lipschitz open set, and let $\Omega_L$ be the union of two translates of $D$
placed symmetrically about the origin, with centers separated by $L$.  For the
restricted fractional Laplacian $(-\Delta)^s$, $0<s<1$, let $\mu_1$ be the one-well ground-state
eigenvalue, let $\phi_1>0$ be $L^2$-normalized, and set
$m_1=\int_D\phi_1$.  We prove
\[
 \lambda_{1,2}(\Omega_L)=\mu_1\mp c_{d,s}m_1^2L^{-d-2s}
 +O(L^{-d-2s-2}+L^{-2d-4s}),
\]
where the upper and lower signs correspond to $\lambda_1$ and $\lambda_2$,
respectively.  The proof uses an exact reduction by central inversion, an
isolated-eigenvalue perturbation estimate, and a second-order expansion of the
interaction kernel.  For two
equal balls, the expansion for $\lambda_2$ gives the leading convergence rate
in the nonlocal Hong--Krahn--Szeg\H{o} minimizing sequence.  We also consider
$\Omega_{\mathbf a,L}=\bigcup_{j=1}^N(D+La_j)$ for a fixed finite family of
identically oriented translates of a common well $D$, assumed bounded and
Lipschitz, where central symmetry is not assumed and the sites
$a_1,\ldots,a_N$ are distinct.  The first $N$ eigenvalues satisfy
...
$-c_{d,s}m_1^2\lvert a_i-a_j\rvert^{-d-2s}$.  Fixed-mesh Galerkin computations
using exact cell integrals illustrate the two- and three-well limits.
...
We prove that two distant translates of one centrally symmetric component
exhibit a nonzero splitting that decays algebraically with their separation.
...
proved a fractional concentration--compactness trichotomy for bounded Sobolev sequences and
transferred its dichotomy to quasi-open domains through operator-norm
...
The cited works do not derive a large-separation spectral expansion for the
standard restricted fractional Laplacian.  Zahl's discrete energy anticipates
the matrix in \eqref{eq:effective-matrix}; Theorem~\ref{thm:multi-well}
derives that matrix from the full operator, with operator-norm control of the
off-diagonal block and spectral-gap control of the complementary modes, and
identifies every ordered cluster eigenvalue.  We are not aware of an earlier
result giving either the two-component coefficient with a quantified
remainder or this multi-well spectral reduction.

Our contribution has three parts.  First, Theorem~\ref{thm:two-well}
identifies the leading coefficient and sign of both eigenvalue shifts for two
centrally symmetric wells, with a quantified remainder.  Second,
Theorem~\ref{thm:multi-well} identifies the entire ground-state cluster for
any fixed finite family of identically oriented translates with the spectrum
of an explicit interaction matrix and gives uniform cluster-error bounds.
Third, Section~\ref{sec:numerics} gives exact-cell-integral, fixed-mesh
computations illustrating both limits.

Central symmetry permits the exact
decomposition into the scalar symmetric and antisymmetric branches
$A_D\pm B_L$.  In the symmetry-free multi-well compression, the corresponding
first-order kernel term instead cancels algebraically after projection onto the
one-well ground states.  Its $N=2$ specialization,
Corollary~\ref{cor:symmetry-free-two}, therefore gives the same leading
doublet coefficient without symmetry.  Both proofs are self-contained after
the standard one-well spectral facts.
...
positive rescaling
\cite[Section~1.1, p.~2]{BrascoParini2016}.  The form norm
...
inversion identify the two diagonal parts of the form with $\mathcal q_D[u]$ and
$\mathcal q_D[v]$.
...
variables, expanding the square in \eqref{eq:form} yields the exact identity
\begin{align*}
...
We finish with a conforming Galerkin check at fixed mesh for the two- and
three-well limits.  The computation is not used in the proofs, and no
$h\to0$ discretization-error estimate is claimed; it tests only the proved
$L\to\infty$ asymptotic.  Take $d=1$, $0<s<1/2$, and consider finite unions
of unit intervals.  For analytic one-well results on an interval, see
...
fixed-mesh effective-matrix spectra are
\begin{align*}
...
$L$ & $L^{3/2}(\lambda_{1,h}-\mu_{1,h})$
    & $L^{3/2}(\lambda_{2,h}-\mu_{1,h})$ & maximum error\\ \hline
...
$L$ & $L^{3/2}(\lambda_{1,h}-\mu_{1,h})$
    ...
    & $L^{3/2}(\lambda_{3,h}-\mu_{1,h})$ & maximum error\\ \hline
...
\hline
\end{tabular*}
\caption{Scaled cluster shifts on the 180-cell-per-component mesh for two
wells centered at $(-L/2,L/2)$ and three wells centered at $(-L,0,L)$;
the maximum error is measured against the corresponding effective-matrix
spectrum.}
\label{tab:clusters}
\end{minipage}
\par\medskip
...
\begin{sloppypar}
The ancillary file \path{check_asymptotics.py} reproduces
...
the exact reuse of each translated one-well block, then prints progress after
each separation.  The pinned numerical dependencies are listed in the
ancillary file \path{requirements-numerics.txt}; the reference output in
\path{asymptotics_180.txt} was replayed with
Python~3.12.3, NumPy~1.26.4, and SciPy~1.11.4.  The submission source package
includes a frozen copy of the script, pinned dependencies, and reference
output; these archived files correspond to the manuscript.
The version tag for this manuscript is
\texttt{paper-2026-09-04-r5}.  The tagged development source is available at
\href{https://github.com/skypher/laplace-tunneling/tree/paper-2026-09-04-r5}%
{github.com/skypher/laplace-tunneling/tree/paper-2026-09-04-r5}.
\end{sloppypar}
