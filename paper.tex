...
\cite[Section~10, equations~(10.3) and~(10.7)]{Zahl2026}.  That discrete
energy anticipates the matrix in \eqref{eq:effective-matrix}; the result here
derives it from the full restricted fractional operator, bounds the
off-diagonal block in operator norm, controls its coupling to the
complementary one-well modes through the spectral gap, and identifies every
ordered eigenvalue in the cluster.  We are not aware of an earlier result giving either the
two-component coefficient with a quantified remainder or this finite-well
spectral reduction.
...
centers are separated by $L$.  Then
\[
...
$\gamma_L=\beta_L$ and the two displayed remainder bounds coincide.
\end{proof}
...
$\theta_N>0$.  The assertions for the full operator follow from
\eqref{eq:multi-eigenvalue-bound} and the strict gaps between the relevant
eigenvalues of $M_{\mathbf a}$.
\end{proof}
...
$\lambda_{k,h}(L)$.  Then, for fixed distinct $a_1,\dots,a_N$,
