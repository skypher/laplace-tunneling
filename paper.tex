...
and interchanged by reflection, whose centers are separated by $L$.  For the
...
principal eigenvalue as the separation varies, including continuity when the
patches meet \cite[Theorem~1]{LeculierRoquejoffre2023}.  Their result concerns
the principal eigenvalue across all separations but does not identify its
large-separation coefficient or the accompanying second-eigenvalue split.
Together these works establish the qualitative limiting, continuity, and
dichotomy picture but do not specify the leading interaction coefficient for
the standard restricted fractional Laplacian.  The purpose of this article is
to extract that coefficient and the associated effective finite-dimensional
interaction.
...
Consequently, for all sufficiently large $L$, $\lambda_1(\Omega_{\mathbf
a,L})$ is simple and
...
$L^\kappa r_L\to0$.  Since $\theta_2-\theta_1>0$, choose $L$ so large that
\[
 L^\kappa r_L
...
\texttt{paper-2026-09-03}.  The development repository is available at
