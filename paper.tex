...
\newcommand{\shortauthors}{RESTRICTED FRACTIONAL LAPLACIAN}
...
computations on two and three intervals reproduce these limits.
...
\cite[Theorems~1.2 and~1.3]{PariniSalort2020}.  Those results identify the
limit but do not specify the leading interaction coefficient.  The purpose of
this article is to extract that coefficient and the associated effective
finite-dimensional interaction.
...
$\mathcal q_D[v]$.  Because the two components have positive separation,
their cross-kernel is bounded; consequently this unitary maps
$\Hs(\Omega_L)$ onto $\Hs(D)\oplus\Hs(D)$.  Expanding the square in
...
$\mathcal A_0$.  Positive separation makes every cross-kernel bounded, so
the form domain is identified with $\bigoplus_{j=1}^N\Hs(D)$.  For $i<j$,
direct expansion of the form
...
We finish with a conforming Galerkin check of the two- and three-well limits.
...
Let $I_\alpha$ be pairwise disjoint cells of length $h$, let $x_\alpha$ be
their centers, put $r_{\alpha\beta}=\abs{x_\alpha-x_\beta}$, and set
$q=1-2s$.  In the $L^2$-orthonormal basis
$e_\alpha=h^{-1/2}\mathbf 1_{I_\alpha}$, the Galerkin matrix $H_h$ of the
zero-exterior form has entries
\begin{align}
 (H_h)_{\alpha\alpha}
...
Fix a uniform partition of $D=(-1/2,1/2)$ and let $\mu_{1,h}$ and
$\phi_{1,h}$ be the lowest eigenvalue and a positive, $L^2$-normalized
piecewise-constant eigenfunction of its Galerkin matrix.  Put
...
On the 180-cell mesh, the predicted two-well scaled levels are
$(-0.18521477,0.18521477)$.  For three collinear centers
$(-L,0,L)$, they are
$(-0.29671332,0.06548331,0.23123001)$.  The direct Galerkin eigenvalues are
shown in Table~\ref{tab:clusters}; the last column is the largest absolute
difference from the corresponding effective-matrix eigenvalue.
...
\path{requirements-numerics.txt}.
