...
the finite-well result: for any bounded Lipschitz reference domain and any
fixed collection of distinct centers $a_1,\dots,a_N$, the first $N$
eigenvalues are
\[
...
proved the mixed local/nonlocal analogue
\cite[Theorem~1.1]{BiagiDipierroValdinociVecchi2023}.  Parini and Salort placed
such distant components in a general concentration--compactness dichotomy
\cite[Theorems~1.1 and~1.2]{PariniSalort2020}.  Together these works establish
the qualitative limiting and dichotomy picture but do not specify the leading
interaction coefficient for the ordinary restricted fractional Laplacian.
The purpose of this article is to extract that coefficient and the associated
effective finite-dimensional interaction.
...
None of these works derives a large-separation spectral expansion for the ordinary
...
explicit weighted interaction matrix; unlike the reflection reduction, this
result does not require central symmetry of the reference component.  Both
proofs are self-contained after the standard one-well spectral facts.  The
interval provides an explicit benchmark: one-well eigenvalue and
...
Assume in addition that $D=-D$.  For $N=2$, $a_1=-e/2$, and $a_2=e/2$, the eigenvalues of
...
vertices of a regular simplex.  Such a configuration in $\R^d$ necessarily
has $N\leq d+1$.  Put
\[
...
The proof architecture was developed with assistance from OpenAI's GPT-5.6
...
\path{requirements-numerics.txt}.  A frozen copy of the scripts, pinned
dependencies, release audit, and reference output is included in the
submission source package.  The development repository is available at
