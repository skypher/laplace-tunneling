...
Lipschitz open set, and let $\Omega_L$ be the union of two symmetric translates
of $D$ whose centers are separated by $L$.  For the restricted fractional
Laplacian $(-\Delta)^s$, $0<s<1$, let $\mu_1$ be the one-well ground-state
...
\[
 ...
 +O(L^{-d-2s-2}+L^{-2d-4s}),
\]
using an exact central-inversion reduction, an isolated-eigenvalue perturbation
estimate, and a second-order expansion of the interaction kernel.  For two
equal balls, the upper member of the doublet gives the leading rate in the
nonlocal Hong--Krahn--Szeg\H{o} minimizing sequence.  We also treat fixed
finite families of identically oriented translates of one well, with no
...
\cite[Theorem~6.2]{BrascoParini2016}.  Biagi, Dipierro, Valdinoci, and Vecchi
proved a mixed local/nonlocal analogue
\cite[Theorem~1.1]{BiagiDipierroValdinociVecchi2023}.

A second analogue is due to Goel and Sreenadh; it treats a local
$p$-Laplacian coupled to a nonlocal $p$-Laplacian with a compactly supported kernel
\cite[Theorem~1.4]{GoelSreenadh2019}; the latter interaction is finite-range
rather than algebraic.  Parini and Salort proved a fractional
concentration--compactness trichotomy for bounded Sobolev sequences and
...
Our contribution has three parts.  First, we identify the leading coefficient
and sign of the first two eigenvalue shifts for two centrally symmetric wells.
Second, we derive an explicit effective Hamiltonian for finitely many
identically oriented translates of a common well, without a symmetry
assumption.  Third, we give uniform spectral-cluster errors and fixed-mesh
...
explicit weighted interaction matrix.  Central symmetry permits the exact
...
the standard one-well spectral facts.  For a one-dimensional interval,
one-well eigenvalue and eigenfunction estimates are available in
...
sources write the operator order as $\alpha=2s$.
...
and every $L\geq L_0$ from \eqref{eq:explicit-L0}, the error expression
$\varepsilon_L+2\gamma_L^2/g$ in \eqref{eq:multi-eigenvalue-bound} is exactly
the right-hand side of each bound
...
and consider finite unions of unit intervals.  Partition each component into
cells of length $h$.  The cell
...
\hline
\end{tabular*}
\caption{Scaled two-well (top) and three-well (bottom) cluster shifts at
...
Every $L\to\infty$ limit keeps all other data fixed, including $N$ and the
distinct sites $\mathbf a$ in Section~4 and the translated mesh in Section~5.
The multi-well construction in Section~4 uses identically oriented translates
$D+La_j$ of the common reference well.
...
\begin{sloppypar}
The ancillary script \path{anc/check_asymptotics.py} reproduces
...
each separation.  The pinned numerical dependencies are
listed in
\path{anc/requirements-numerics.txt}; the reference output in
\path{anc/asymptotics_180.txt} was replayed with
...
\texttt{paper-2026-09-04-r3}.  The tagged development source is available at
\href{https://github.com/skypher/laplace-tunneling/tree/paper-2026-09-04-r3}%
{github.com/skypher/laplace-tunneling/tree/paper-2026-09-04-r3}.
\end{sloppypar}
